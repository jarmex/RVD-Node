...
const {
  debug,
  printerror
} = (0, _logger.default)().getContext('ctrlStep');

let ControlSteps = function ControlSteps() {
  _defineProperty(this, "process", async (step, appdata = {}, tempdata = {}, moduleName) => {
    ...
    const {
      ...
      actions: reAction,
      conditionExpression
    } = step; // get the conditionExpression

    const conditionResults = {}; // loop through to check all the conditions are met
    ...
    if (conditions) {
      ...
      conditions.forEach(cod => {
        ...
        const first = (0, _ReplaceVariables.default)(operand1, data, temp);
        const second = (0, _ReplaceVariables.default)(operand2, data, temp);
        let op1 = first;
        let op2 = second;

        if (mtype === _Constants.ConType.numeric) {
        ...
        switch (cod.operator) {
          case _Constants.ConOperator.eq:
            conditionResults[name] = op1 === op2;
            break;

          case _Constants.ConOperator.gt:
            conditionResults[name] = op1 > op2;
            break;

          case _Constants.ConOperator.lt:
            conditionResults[name] = op1 < op2;
            break;

          case _Constants.ConOperator.gte:
            conditionResults[name] = op1 >= op2;
            break;

          case _Constants.ConOperator.lte:
            conditionResults[name] = op1 <= op2;
            break;

          case _Constants.ConOperator.ne:
            conditionResults[name] = op1 !== op2;
            break;
        ...
        debug(`${cod.name}: ${op1} "${cod.operator}" ${op2} = ${conditionResults[name]}`);
      });
    }

    debug('%o', conditionResults); // check for the condition expression

    if (conditionExpression) {
      debug(conditionExpression);
      const conditionSplit = conditionExpression.toLowerCase().split(' ');
      let prevAnd = false;
      let prevOr = false; // C1 AND C2 AND C3 => c1 and c2 and c3 => ['c1', 'and', 'c2', 'or', 'c3' ]
      ...
      conditionSplit.forEach(nameOrcond => {
        if (nameOrcond === 'and') {
          ...
          const resultExist = conditionResults[nameOrcond]; // check if result computed exist
    ...
    debug(`isValid = ${isValid}`); // take Actions

    if (isValid) {
      const actions = (0, _ensureArray.default)(reAction); // eslint-disable-next-line

      for (let j = 0; j < actions.length; j++) {
        const act = actions[j];

        if (act.assign) {
          const expr = (0, _ReplaceVariables.default)(act.assign.expression, data, temp);
        ...
        if (act.capture) {
          try {
            ...
            const strdata = (0, _ReplaceVariables.default)(capdata, data, temp);
...
//# sourceMappingURL=ControlSteps.js.map